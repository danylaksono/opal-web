...
/**
 * Checking review annotations against a freshly compiled PDF (ADR-010).
 *
 * Ported from `opal-editor@8f95519`'s `review-reanchor.ts`, unchanged in
 * behaviour: annotations are stored as page + x/y boxes measured against one
 * exact build, so a recompile that reflows text leaves them pointing
 * somewhere else. This pass reports that; it does not repair it. An
 * annotation stays exactly where it was placed, because that position is a
 * deliberate act — the reader put the box where they meant it, over a figure,
 * a margin, a region — and re-seating it on whatever the text search matched
 * replaces their judgement with a guess. A confidently wrong new position is
 * worse than a stale one: it looks authoritative. So the pass answers "is
 * this still true?" and leaves acting on the answer to the person.
 *
 ...
 *     than a span — and only as good as the stored line number, which the
 *     user may since have edited around. But it is the only route for
 *     annotations with no text under them.
 *
 * Four outcomes: the text is where the annotation is (ok), it is somewhere
 * else and we know where (shifted), it is gone (drifted), or there was
 * nothing to search by in the first place (unverified) — which is not
 * evidence either way, and calling it drift would cry wolf over every old
 * annotation.
 *
 * `ReanchorContext` is the only thing that differs from desktop: this port
 * binds `searchPage` and `forwardSearch` against no implementation yet.
 * `src/workers/pdf/protocol.ts` has no `searchPage` message — only
 * `pageText` — and `forwardSearch` needs SyncTeX, which PLAN.md's "next, in
 * order" marks unproven pending its own item. Wiring either into
 * `PreviewPane` is deliberately not part of this change.
 */

/** Shorter needles than this match half the document. */
const MIN_ANCHOR_TEXT = 6;
/** Handed to the renderer as the needle. Long multi-line needles miss more
 *  often than they help; the opening words are enough to identify a span. */
const PRIMARY_NEEDLE_CHARS = 60;
/** Retried on the original page only, for text reflowed enough that the
 *  longer needle no longer matches. */
const FALLBACK_NEEDLE_CHARS = 24;
/** How far from the stored page to look. Edits shift content by a page or
 *  two; sweeping the whole document per annotation would be O(pages x
 *  notes). */
const SEARCH_RADIUS_PAGES = 8;
/** The renderer allocates the hit array up front, and only the first hit is
 *  used. */
...
/** PDF points. Roughly a line of body text: rebuilds nudge things by
 *  fractions of a point, and a highlight drawn a little above its words
 *  should not be reported as having moved. */
...
export interface ReanchorContext {
  pageCount: number;
  /** pageIndex is 0-based, matching the renderer worker protocol. */
  searchPage(
    pageIndex: number,
    needle: string,
    maxHits: number,
   ...
   *  Batched because resolving one location means reading and parsing the
   *  whole synctex file — minutes of work if repeated per annotation. */
  forwardSearch(
...
/** Collapse the whitespace a renderer reports between lines so the needle
 *  still matches text that has since been rewrapped. */
...
/** Bounding box over every rectangle a match covers — a match wrapping
 *  across lines reports one rect per line. */
function boundsOf(rects: readonly Rect[]): Omit<ReanchorBox, "page"> | null {
...
async function relocateByText(
  ...
  // Last resort, and only where the annotation started: a short needle over
  // the whole radius would happily match an unrelated paragraph two pages
  // away.
 ...
 *  dragged highlight is whatever box the reader swept out, which has no
 *  reason to match the bounding box of the words inside it, and treating
 *  that difference as movement would report drift on every annotation ever
 *  made with the highlighter. */
function classify(
  id: string,
  anchor: ReviewAnchor,
  box: ReanchorBox,
...
export async function reanchorAnnotations(
  ...
  for (const comment of comments.slice(0, MAX_REANCHOR_ANNOTATIONS)) {
    ...
    // A search abandoned mid-flight proves nothing — classifying it now
    // would report drift that a newer build has already made irrelevant.
    if (ctx.isCancelled?.()) return updates;
    if (box) {
