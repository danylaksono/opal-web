# Opal Web: the original architecture investigation

Written 2026-07-23 against desktop Opal v1.4.8, before this repository could
compile anything. It is kept whole and unedited, for two reasons:

1. **The reasoning is the value.** Every decision since was made from it or
   against it, and a conclusion without its argument is a thing nobody can
   revisit.
2. **The code cites it.** Comments across `src/` name sections of this file —
   `investigation.md 14` is the delivery roadmap, `7.3` the TeX resource
   policy, `6.1` the storage split. The numbering here is that numbering.

Where measurement has since contradicted it, `PLAN.md` says so; this file is
not updated to match. `docs/progress-archive.md` holds the measurements that
did the contradicting.
...
> here: a directory mirror (investigation.md 6.3 defers it) and history snapshots, which
